...
function Scratcher(canvasId, coverImg, babyImg, canvasWidth, canvasHeight) {
	...
	this.coverImg = document.createElement('img'); // ×ãÇò±¦±´
...
Scratcher.prototype.scratchLine = function(x, y, fresh) {
	this.drawctx.lineWidth = 50; //±Ê´¥¿í¶È
	this.drawctx.lineCap = this.drawctx.lineJoin = 'round';
	this.drawctx.strokeStyle = '#111'; //²»ÄÜÓÃ#000
	if (fresh) {
...
//获取canvas要设置的宽高
var width = parseInt($(window).width() * 1),
	height = parseInt(width * 0.45);


var scratcher = new Scratcher('scratcher',data[0].babyImg,data[0].coverImg,width,height);
